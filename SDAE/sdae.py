    ...
    def get_pretrained_sda(self, data_in, data_val, data_test, dir_out, get_enc_model=True, write_model=True,
        ...
        for cur_layer in range(self.n_layers):
            ...
            if model_layers[cur_layer] is None:
                ...
                #                       bias=self.bias)
                encoder_layer = Dense(activation=self.enc_act[cur_layer], name="encoder" + str(cur_layer), units=self.n_hid[cur_layer],
                                      kernel_initializer="glorot_uniform", use_bias=self.bias)
